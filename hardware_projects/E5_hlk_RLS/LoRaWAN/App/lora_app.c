...
#define LogDataSize 16

extern volatile uint8_t corr_error_code;
extern volatile uint8_t kalman_error_code;

extern uint32_t global_radar_distance[SAMPLING_DATA];
extern uint32_t current_distance ;
uint16_t LogTxData[log_size] ={0};
extern uint32_t kalman_distance;
...
//static void batteryReadError(void *argument);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  switch (GPIO_Pin)
  {
    default:APP_PRINTF("Unknown Interrupts \r\n");
      break;
  }
}
...
//	    for (int k=0;k<16;k++){
//
//		    AppData.Buffer[i++] = (LogTxData[k] >> 8) & 0xFF;
//		    AppData.Buffer[i++] = LogTxData[k] & 0xFF;
//	    }
//	    AppData.Buffer[i++] = (kalman_distance >> 24) & 0xFF;
//	 	AppData.Buffer[i++] = (kalman_distance >> 16) & 0xFF;
//	 	AppData.Buffer[i++] = (kalman_distance >> 8) & 0xFF;
//	 	AppData.Buffer[i++] = kalman_distance & 0xFF;
//	 	AppData.Buffer[i++] = kalman_error_code & 0xFF; // The error code byte
//
//        //3. Log Data Filling
//
//
//
//
//	    LogTxData[logIndex]=(uint16_t)current_distance;
//
//	    logIndex = (logIndex + 1) % LogDataSize;
...
//  AppData.Buffer[i++] = (sensor_data.temperature >> 8 ) & 0xFF;
//  AppData.Buffer[i++] = sensor_data.temperature & 0xFF;
//  AppData.Buffer[i++] = (sensor_data.humidity >> 8 ) & 0xFF;
//  AppData.Buffer[i++] = sensor_data.humidity & 0xFF;
//  AppData.Buffer[i++] = (sensor_data.battery_voltage >> 8 ) & 0xFF;
//  AppData.Buffer[i++] = sensor_data.battery_voltage & 0xFF;
