...
MODEL_SAVE_PATH = "c:/Users/sahil/Desktop/ICFOSS/Anomaly Detection/models/saved/large_ar_mlp_reg_opt.keras"
TFLITE_SAVE_PATH = "c:/Users/sahil/Desktop/ICFOSS/Anomaly Detection/models/saved/large_ar_mlp_reg_opt.tflite"
...
def train(model, X_train, y_cls_train, y_reg_train, reg_mask_train,
    ...
    def train_step(xb_seq, ycb_seq, yrb_seq, rmb_seq):
        ...
        with tf.GradientTape() as tape:
            ...
            total_cls = total_cls / tf.cast(T, tf.float32)
            total_reg = total_reg / tf.cast(T, tf.float32)
            
        grads = tape.gradient(loss, model.trainable_variables)
        optimizer.apply_gradients(zip(grads, model.trainable_variables))
        return loss, total_cls, total_reg
    ...
    print("Epoch | Train Loss | Val Loss  | Val Reg Loss | EarlyStop")
    print("-" * 58)
    ...
    for epoch in range(epochs):
        ...
        for i in range(n_batches):
            ...
            batch_loss, _, _ = train_step(xb, ycb, yrb, rmb)
        ...
        for i in range(v_batches):
            ...
            yrb = yr_v[start:end]
            rmb = rm_v[start:end]
            batch_loss, _, batch_reg = val_step(xb, ycb, yrb, rmb)
            v_total_loss += batch_loss
            v_total_reg += batch_reg
        
        val_loss = v_total_loss / v_batches
        val_reg_loss = v_total_reg / v_batches
        ...
        if (epoch + 1) % 5 == 0 or epoch == 0 or patience_cnt >= PATIENCE:
            print(f"{epoch+1:>5} | {train_loss:>10.4f} | {val_loss:>9.4f} | {val_reg_loss:>12.4f} | {patience_cnt}/{PATIENCE}")

        if patience_cnt >= PATIENCE:
            print("Early stopping triggered (based on regression loss).")
            break
